...
contract CommunityLilypad is Ownable {
    ...
    ModicumContract remoteModicumInstance;
    ...
    constructor(
        ...
        address _modicumContract
    ) {
        require(
            _modicumContract != address(0),
            "Community: contract cannot be zero address"
        );
        treasury = _treasury;
        // i_link = _link;
        // registrar = _registrar;
        // i_registry = _registry;
        remoteModicumInstance = ModicumContract(_modicumContract);
    }
    ...
    function runModule(
        ...
        return
            remoteModicumInstance.runModuleWithDefaultMediators{
                value: msg.value
            }(name, params);
    }
    ...
    function receiveJobResults(uint256 _jobID, string calldata _cid) public {
        ...
        jobIdToResultUrl[_jobID] = jobResult.httpString;

        emit ReceivedJobResults(_jobID, _cid);
    }

    function getParticularResultUrl(
        uint256 _jobId
    ) public view returns (string memory url) {
        url = jobIdToResultUrl[_jobId];
    }
